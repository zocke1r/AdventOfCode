...
function mapToGame(gameString : string) : game{
    
    let game = {
        id: Number(/Game (\d+)/.exec(gameString)[1]),
        sets : mapToSet(gameString.substring(gameString.indexOf(":")+1))
    }
...
function mapToSet(setString: string): set[]{
    return setString
        .split(";")
        .map(s => {
            return {
                blue : /(\d+) blue/.test(s) && Number( /(\d+) blue/.exec(s)[1]) || 0,
                red : /(\d+) red/.test(s) && Number( /(\d+) red/.exec(s)[1]) || 0,
                green : /(\d+) green/.test(s) && Number( /(\d+) green/.exec(s)[1]) || 0,
            }
        })
...
function isSetPossible(set : set): boolean {
    return set.blue <= maxValue["blue"] && set.green <= maxValue["green"] && set.red <= maxValue["red"];
}

function isGamePossible(game: game) : boolean{
    return game.sets.map(isSetPossible).reduce((a,b) => a && b)
}



function minCubes(game : game){
    return {
        "green" : Math.max(...game.sets.map(g => g.green)),
        "red" : Math.max(...game.sets.map(g => g.red)),
        "blue" : Math.max(...game.sets.map(g => g.blue))
    }
}
